...
; MOVSX
; movsx ; Converts the type of the second operand to the frist one
...
; add sets CF if overflow has ocurred
...
; cmp ; This really substracts the source op to the destination but only set the flag and don't alter the operands
; neg ; This really substracts a signed zero so the sing value becomes negative or positive
...
; The divident (accumulator) is twice size of the divisor (operand) & the quotient & remainder have the same size as the divisor
; If the divisor is byte size the divident is taken from ax , the quotient is stored in al & remainder in ah
; If the divisor is word size the upper half of divident is taken from edx, the lower half is taken from eax. The quotient is stored in eax & remainder is stored in edx
; idiv ; Performs a signed division accumulator/operand
; It uses the same register as div

; ARITHMETIC INSTRUCTIONS
; Decimal arithmetic instructions are used to adjust the results of a pervious binary arithmetic operand to produce a valid packed or unpacked decimal result
...
; and, or , xor ; This intruction works like logical ones
...
; btc word [bx], cx ; This sets CF to the bx cell of cx value and the resplace that cell value for the component value
...
; -- exemples ;
; shl al,1 ; Shift al regsiter left by 1 bit (this move the register)
...
; shld ; Shifts destination operand by the value of the third operand and fill the empty cells with the soruce operand
; -- examples ;
...
; rotation the lower & upper of the destination operand are swaped and the the scope goes to upper - 1 & lower + 1
end